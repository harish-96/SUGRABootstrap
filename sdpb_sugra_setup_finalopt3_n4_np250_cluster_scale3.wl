...
ClearAll[SpwOPT];
SpwOPT[\[ScriptL]_][maxdegree_]:=Block[{tpwReduced,tpw,int1Reg,int2RegOPT,constraintsgrid},
...
(*constraintsgrid=Range[1,numPTS,4];*)
...
tpw=( (\[ScriptS][\[Rho]gridCheb]^7)/(2^18 3 \[Pi]^4))({(2^18 3 \[Pi]^4)/\[ScriptS][\[Rho]gridCheb]^3 1/(2^18 3 \[Pi]^4)(2^10 3)/Pochhammer[\[ScriptL]+1,6]}~Join~(BasisMandelstam[maxdegree]premon//ExpandAll//#/.mon[x_,y_,z_]pmon[a_,b_,c_]:> int2RegOPT[\[ScriptL]][x+a,y+b,z+c]&) //Transpose);
...
ImAmplitudeReduced[nmax_]:=ImAmplitudeReduced[nmax]=Table[ImAmplitude[nmax][[i]] . \[Alpha]abc[nmax]/.HEConstraintModuleOpt[nmax]//Expand//Table[Coefficient[#,x],{x,\[Alpha]abcReduced[nmax]}]&//Join[#,{0}]&,{i,1,numHSC}];
...
Module[
{objective,\[ScriptL]grid,\[ScriptL]gridinfty,constraintsgrid,
\[Alpha]vecs,\[Alpha]vecsinfty,cvals,\[Beta]vecsHE,\[Beta]vecsHS,\[Beta]scale,\[Beta]impart},
...
Run["mpirun -v -np 160 pvm2sdp 1024 "<>fname<>".xml ./"<>fname ]; 
DeleteFile[fname<>".xml"];

Print["sdpb start ..."];

Run["mpirun -v -np 160 sdpb -s ./"<>fname<>" --noFinalCheckpoint --checkpointInterval 50000  --precision 1000 --procsPerNode 40  --maxRuntime 1072800 --maxIterations 100000 --initialMatrixScalePrimal 1e30 --initialMatrixScaleDual 1e30 --dualityGapThreshold 1e-15 --maxComplementarity 1e100 >> "<>fname<>".log"];
Run["rm -r "<>fname];
];
...
res=If[FileExistsQ["/gpfs/hmurali/string_bootstrap/SUGRABootstrap3/"<>fname<>"_"<>ToString[Lmax]<>"_"<>ToString[Nmax]<>"_"<>ToString[Linfty]<>"_out/y.txt"],
Import["/gpfs/hmurali/string_bootstrap/SUGRABootstrap3/"<>fname<>"_"<>ToString[Lmax]<>"_"<>ToString[Nmax]<>"_"<>ToString[Linfty]<>"_out/y.txt","Table"],
