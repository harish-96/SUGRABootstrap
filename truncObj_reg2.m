...
*)
nnodes = ToExpression@Environment["SLURM_JOB_NUM_NODES"];
ncpus = ToExpression@RunProcess[{"nproc","--all"}, "StandardOutput"];
...
(*constraintsgrid = Range[numPTS];*)
constraintsgridImT=Range[numPTS];
(*constraintsgridImT=Join[Range[1,numPTS,3],Range[numPTS-100,numPTS,1]]//DeleteDuplicates//Sort;*)
...
ClearAll[matrix];
ClearAll[matrix];
matrix[nmax_]:=matrix[nmax]=Block[{filename, mat},filename="/gpfs/hmurali/string_bootstrap/transferMatrices/matrix"<>ToString@nmax<>".txt";
If[FileExistsQ[filename], mat=ToExpression@Import[filename],
mat=\[Alpha]abc[nmax]/.HEConstraintModuleOpt[nmax]//Table[Table[Coefficient[#[[kk]],x],{x,\[Alpha]abcReduced[nmax]}],{kk,1,Length[#]}]&;
Export[filename,ToString@FullForm@mat]]; mat];
...
int1Reg[l_][b_,c_]/;b<=c:=int1Reg[l][b,c]=ToExpression[If[FileExistsQ["/gpfs/aguirrieri/integral_storage_SUGRA2/integral_storage_10d/int1Reg_10d_grid1_"<>ToString[l]<>"_"<>ToString[b]<>"_"<>ToString[c]<>".txt"],
Import["/gpfs/aguirrieri/integral_storage_SUGRA2/integral_storage_10d/int1Reg_10d_grid1_"<>ToString[l]<>"_"<>ToString[b]<>"_"<>ToString[c]<>".txt"],
Import["/gpfs/hmurali/string_bootstrap/integral_storage/int1Reg_10d_grid1_"<>ToString[l]<>"_"<>ToString[b]<>"_"<>ToString[c]<>".txt"]]]//Chop[#,10^-prec]&;
...
ClearAll[target];
target[nmax_]:=target[nmax]=Block[{targetp,s\[ScriptS],t\[ScriptT],u\[ScriptU]},targetp=(Amplitude[\[Epsilon] s\[ScriptS],\[Epsilon] t\[ScriptT],\[Epsilon] u\[ScriptU]][nmax]//SeriesCoefficient[#,{\[Epsilon],0,4},Assumptions->s\[ScriptS]>0&&t\[ScriptT]<0&&u\[ScriptU]<0]&//#/s\[ScriptS]^4 &) . matrix[nmax];Join[targetp,{0}]];
...
HSConstraintReduced[nmax_]:=HSConstraintReduced[nmax]=Table[Join[i[[;;-2]] . matrix[nmax], {0}], {i, HSConstraintOPT[nmax]}];


ClearAll[ImAmplitudeReduced];
ImAmplitudeReducedAll[nmax_]:=ImAmplitudeReducedAll[nmax]=Table[Join[ImAmplitude[nmax][[i]] . matrix[nmax], {0}],{i,1000}];
ImAmplitudeReduced[nmax_]:=ImAmplitudeReduced[nmax]=Table[Join[ImAmplitude[nmax][[i]] . matrix[nmax], {0}],{i,constraintsgridImT}];
...
writeQuadraticProgram[file_,obj_,qc[\[Alpha]vecs_,\[Beta]vecs_, Mreg_]] :=
...
regulatorconstraints=Table[
idi = almostzeroarray[Length[\[Alpha]vecs[[1]]], i -> 1];
PositiveMatrixWithPrefactor[1, {{Mreg norm, idi}, {idi, Mreg norm}}], {i,2,Length[\[Alpha]vecs[[1]]]-1}];

WriteBootstrapSDP[file,SDP[obj,norm,quadraticconstraints~Join~linearconstraints~Join~regulatorconstraints]];
]


ClearAll[buildSmatrixprogram]
buildSmatrixprogram[file_][\[ScriptL]max_,nmax_,Linfty_,mreg_]:=
Module[
...
fullfls = ParallelTable[SpwOPT[\[ScriptL]][nmax],{\[ScriptL],\[ScriptL]grid}];
\[Alpha]vecs = Table[fullfls[[\[ScriptL],i,All]], {\[ScriptL],1,Length@\[ScriptL]grid},{i,constraintsgrid}]//Flatten[#,1]&;
...
writeQuadraticProgram[file,objective,qc[\[Alpha]vecs~Join~\[Alpha]vecsinfty,\[Beta]vecsHS~Join~\[Beta]scale~Join~\[Beta]impart, mreg]];
]
...
ClearAll[maxabk];
maxabk[\[ScriptL]max_, nmax_,linfty_,mreg_][fname_]:=
Block[{},
matrix[nmax];
buildSmatrixprogram[fname<>".xml"][\[ScriptL]max,nmax,linfty,mreg];
...
Mreg=ToExpression[$CommandLine[[8]]];
...
integrandsExactNods = 2/Pi Table[1 / sgridHSC[[i]], {i,1000}] (ImAmplitudeReducedAll[Nmax] . res2);
integrandsExact = 2/Pi Table[(dsgridHSC / sgridHSC)[[i]], {i, 1000}] (ImAmplitudeReducedAll[Nmax] . res2);

\[ScriptL]grid = Range[0,Lmax,2];
(*integrandstrunc = Re[(2^19 3 \[Pi]^3 Table[dsgridCheb[[i]]/\[ScriptS][\[Rho]gridCheb[[i]]]^8 ((2^6 Gamma[7/2]^2)/\[Pi] (l!(l+7/2))/Gamma[l+7]) GegenbauerC[l,7/2,1]^2, {l,\[ScriptL]grid},{i, constraintsgrid}]//Flatten)  (Imfls . res2)];*)
