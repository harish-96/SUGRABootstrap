...
constraintsgrid=Join[Range[1,numPTS,3],Range[numPTS-100,numPTS,1]]//DeleteDuplicates//Sort;
...
\[Beta]imsubtract = 2^18 3 \[Pi]^4 Table[sgridCheb[[si]]^-3 Sum[Imflsfull[[li, si, ni]] gammal[[li]] Cl1vals[[li]]^2, {li,1,Length@\[ScriptL]grid}], {si,constraintsgridImT},{ni,Length[\[Alpha]vecs[[1]]]}];
...
integrandstrunc = Re[(2^19 3 \[Pi]^3 Table[dsgridCheb[[i]]/\[ScriptS][\[Rho]gridCheb[[i]]]^8 ((2^6 Gamma[7/2]^2)/\[Pi] (l!(l+7/2))/Gamma[l+7]) GegenbauerC[l,7/2,1]^2, {l,\[ScriptL]grid},{i, constraintsgrid}]//Flatten)  Imfls];
