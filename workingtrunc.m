...
ClearAll[SetScale];
SetScale[nmax_,\[CapitalLambda]_]:=SetScale[nmax]=Join[matrix[nmax][[1]], {-\[CapitalLambda]}];
...
ImAmplitudeReduced[nmax_]:=ImAmplitudeReduced[nmax]=Table[Join[ImAmplitude[nmax][[i]] . matrix[nmax], {0}],{i,1,numHSC}];
...
\[Beta]imsubtract = 2^18 3 \[Pi]^4 Table[sgridCheb[[si]]^-3 Sum[Imflsfull[[li, si, ni]] gammal[[li]] Cl1vals[[li]]^2, {li,1,Length@\[ScriptL]grid}], {si,numPTS},{ni,Length[\[Alpha]vecs[[1]]]}];
...
Run["mpirun -v --oversubscribe -np 240 pvm2sdp 1024 "<>fname<>".xml ./"<>fname ]; 
(*DeleteFile[fname<>".xml"];*)

Print["sdpb start ..."];

Run["mpirun -v --oversubscribe -np 240 sdpb -s ./"<>fname<>" --noFinalCheckpoint --checkpointInterval 9000  --precision 1000 --procsPerNode 80  --maxRuntime 1072800 --maxIterations 100000 --initialMatrixScalePrimal 1e30 --initialMatrixScaleDual 1e30 --dualityGapThreshold 1e-12 --maxComplementarity 1e100 >> "<>fname<>".log"];
(*Run["rm -r "<>fname];*)
];
